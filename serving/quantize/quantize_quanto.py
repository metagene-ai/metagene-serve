...
def parse_args():
    ...
    args.model_dir_4bit = args.model_dir_4bit or "/project/neiswang_1391/MGFM/MGFM-serving/model_ckpts/safetensors/quanto-4bit"
...
def main():
    ...
    del model_quanto_4bit

    print(f"Quanto 8bit quantization with {model_dir} ...")
    model_quanto_8bit = QuantizedModelForCausalLM.quantize(
        model,
        weights=qint8,
        exclude='lm_head')
    model_quanto_8bit.save_pretrained(model_dir_8bit)
    tokenizer.save_pretrained(model_dir_8bit)
    print(f"Quanto quantized model saved to {model_dir_8bit}")
