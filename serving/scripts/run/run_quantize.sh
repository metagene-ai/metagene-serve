...
ST_MODEL_DIR="${MODEL_CKPT_DIR}/safetensors"
 ...
 # Quantize using gptq
 GPTQ_4BIT_CKPT_DIR="${ST_MODEL_DIR}/gptq-4bit"
 GPTQ_8BIT_CKPT_DIR="${ST_MODEL_DIR}/gptq-8bit"
 mkdir -p "${GPTQ_4BIT_CKPT_DIR}/${CKPT_STEP}"
 mkdir -p "${GPTQ_8BIT_CKPT_DIR}/${CKPT_STEP}"
 py_script="${CORE_DIR}/quantize/quantize_gptq.py"
 echo "Running GPTQ quantization with safetensors ckpt ${CKPT_STEP} ..."
 python "${py_script}" \
     --model_dir "${ST_MODEL_DIR}" \
     --model_dir_4bit "${GPTQ_4BIT_CKPT_DIR}" \
     --model_dir_8bit "${GPTQ_8BIT_CKPT_DIR}" \
     --model_ckpt "${CKPT_STEP}"
 ...
 QUANTO_8BIT_CKPT_DIR="${ST_MODEL_DIR}/quanto-8bit"
 mkdir -p "${QUANTO_4BIT_CKPT_DIR}/${CKPT_STEP}"
 mkdir -p "${QUANTO_8BIT_CKPT_DIR}/${CKPT_STEP}"
 ...
 python "${py_script}" \
     ...
     --model_dir_8bit "${QUANTO_8BIT_CKPT_DIR}" \
     --model_ckpt "${CKPT_STEP}"