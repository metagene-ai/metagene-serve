...
if __name__ == "__main__":
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=5,  # Matches provided config
        optim="adamw_torch",  # Using AdamW as specified
        per_device_train_batch_size=32,  # Adjusted to match the batch size of 32
        per_device_eval_batch_size=16,  # Keep as is, or set according to eval requirements
        gradient_accumulation_steps=1,
        warmup_steps=50,  # Matches provided config
        learning_rate=1e-3,  # Matches provided config
        lr_scheduler_type="constant_with_warmup",  # Adjusted to constant LR with warmup
        weight_decay=0.01,  # Matches provided config
        adam_beta1=0.9,  # Specific beta1 for AdamW
        adam_beta2=0.999,  # Specific beta2 for AdamW
        adam_epsilon=1e-8,  # Adjusted to match epsilon
        bf16=True,  # Matches precision config (BF16 mixed precision)
        logging_steps=100,
