...
def parse_args():
    ...
    parser.add_argument("--use_vllm", type=int, required=True)
...
def get_model(model_dir, mode_type, mode_dtype=torch.float32, use_quantized=0, use_vllm=0):
    if use_vllm:
        assert mode_type == "safetensors", "VLLM only supports MGFM models"
        print(f"\nLoading model in VLLM format {mode_dtype}\n")
        model = VLLM(
            model=model_dir,
            task="generate",
            dtype=mode_dtype,
...
def sanity_check_generation(model, model_type, input_ids, use_gt=True, use_vllm=0):
    all_ranks = []
    perfect_samples = []
    ...
    for sample_idx in tqdm(range(0, B)):
        ...
        ctx = BatchEncoding({k: (v[sample_idx: sample_idx + 1, :CTX_LEN] if torch.is_tensor(v) else v) for k, v in input_ids.items()})

        ranks = []
        is_perfect = True  # Flag to check if all ranks are 0

        # for tok_idx in range(min(input_ids.shape[1] - CTX_LEN, GEN_LEN)):
        for tok_idx in range(min(input_ids.input_ids.shape[1] - CTX_LEN, GEN_LEN)):
            # gt_tok = input_ids[sample_idx, CTX_LEN + tok_idx]

            gt_tok = BatchEncoding({k: (v[sample_idx, CTX_LEN + tok_idx] if torch.is_tensor(v) else v) for k, v in input_ids.items()})

            # Break if the current token is a padding token
            if gt_tok.input_ids == 0:
                break
            # if gt_tok["input_ids"] == 0:
            #     break

            if use_vllm:
                assert model_type == "safetensors", "VLLM only supports MGFM models"

                sampling_params = SamplingParams(logprobs=20)
                output = model.generate(ctx, sampling_params)
                logprobs_dict = output[0].outputs[0].logprobs[0]
                try:
                    rank = list(logprobs_dict.keys()).index(gt_tok.input_ids.item())
                except ValueError:
                    rank = 500
            else:
                if model_type == "litgpt":
            ...
            if use_gt:
                _ctx = BatchEncoding({k: (v[sample_idx: sample_idx + 1, CTX_LEN + tok_idx].unsqueeze(1) if torch.is_tensor(v) else v) for k, v in input_ids.items()})
                ctx = BatchEncoding({k: (torch.cat([ctx[k], _ctx[k]], dim=1) if torch.is_tensor(ctx[k]) else ctx[k]) for k in ctx.keys()})
                # ctx = torch.cat([
                #         ctx,
